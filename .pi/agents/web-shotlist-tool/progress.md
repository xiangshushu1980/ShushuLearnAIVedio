...
4. ✅ 用户反馈修复：A0 导入可见 / 实体抽取流程 / 实体库清空（2026-08-12 完成）
5. 用户浏览器验收（http://localhost:5173）
6. 实体卡接入工具 A/B（角色卡注入统一从实体库取）+ 拍摄本 chip 点击→实体详情联动
7. 输入源解析增强 + 侧栏联动（hover/跳转/返回栈，docs/22 开发计划第 4 步）
8. V2（用户提）：外部实体导入/多设定适配（source 已预留）；导入导出、样式打磨
