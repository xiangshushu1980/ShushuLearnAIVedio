...
- 状态：🟡 进行中（2026-08-12 两批跑批完成，待用户目视验收）
...
### 2026-08-12 会话收尾（交接点：下一会话从这里继续）
- **队列已释放**（两批跑批完成，ComfyUI 空闲可给其他线用；MC 节点已生效无需再重启）
- **待用户验收**：output/video/h3_mc_same/ 下 same_mc_chain.mp4 vs same_base_chain.mp4（同场景连续动作，相同段1）——重点看段2 开头 Alya 走步/位置衔接；首批换场景对照在 output/video/h3_mc/
- **用户已知倾向**："貌似 chain 好一点"；但认为换场景看不出衔接、base 动态似乎更好——同场景连续版已出，待判定
- **验收通过后的候选变体**：ctx=5（低档）/39/56（高档）、audio_context_length=0（不跨音频）；跑法=改 mc_cases.json 的 mc 字段重提 same_seg2（runner 断点续跑，clip 2 会覆盖）
- **验收不通过的备选方向**：airlock 时长/构图调整；prompt 时间码对齐复查；链式质量递减观察（长链在自然乐句处重开）
- **进度目录**：已迁移 .pi/tasks/mc-test/（旧 .pi/agents/ 路径失效）
