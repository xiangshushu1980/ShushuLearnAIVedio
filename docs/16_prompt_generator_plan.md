...
7. **输出格式自检 + 失败重试**（2026-08-17 P0 回归实测：ref-en 契约下 ~1/3 偶发 markdown 标题（`## subject_definitions`）+ 丢 non_diegetic_music 段；重跑即合规）：合成层必须做解析校验——字段齐全（三字段/六段式）+ 无 markdown 标题 + 无 `(Sx)` 入 retention + 时间戳递增，失败自动重试 1 次，再失败报错不静默放行。docs/08 落地计划的「规则校验」层由此获得必要性证据
