...
## 后续测试建议（2026-08-04 更新，按优先级）
1. **MotionCache 音频专项**：同 prompt（含明确声音指令）× 有/无缓存，对比音频清晰度/音量 → 决定 MotionCache 去留
2. **fp8 多案例统计**：森林/雨夜/静物 × int8/fp8 各一条，统计画质稳定倾向
3. **中文 prompt 专项**：官方 `<d>` 标签格式（`<d>你好，欢迎来到花园</d>`）触发中文语音/字幕
4. **声音细粒度边界**：鸟鸣等弱指令 → 强指令（loud birds chirping）验证模型声音能力边界
5. **15s 动作丰富度**：复杂动作组合 × 15s，验证长视频动作上限
6. **声音对比基线**：记录 mc vs 无缓存的音频频谱/响度差异（若做专项 1）

## 待确认的默认参数（目测后可固化）
- 分辨率：1024×576（画质均衡）| 768×448（批量快速）
- 步数：14（快速档）| 20（成片档）
