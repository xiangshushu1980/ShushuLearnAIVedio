...
5. **记忆整理（顺手级）**：H3 速度旧条目（mem0 id `00d44594`，5s@1024×576 写 123s）与最终矩阵（165s，int8 20步含加载）并存——若 recall 命中 H3 速度主题，update 旧条目补"含加载"条件或标注指向 docs/09 最终数据；按新铁律（结论性条目强制日期+条件）
